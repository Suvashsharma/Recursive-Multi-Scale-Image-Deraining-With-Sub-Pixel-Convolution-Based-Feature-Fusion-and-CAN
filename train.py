...
parser = argparse.ArgumentParser(description="Fusion can multiscale network")
parser.add_argument("--preprocess", type=bool, default=True, help='run prepare_data or not')
parser.add_argument("--batch_size", type=int, default=4, help="Training batch size")
parser.add_argument("--epochs", type=int, default=100, help="Number of training epochs")
parser.add_argument("--milestone", type=int, default=[33,66], help="When to decay learning rate")
parser.add_argument("--lr", type=float, default=0.001, help="initial learning rate")
parser.add_argument("--save_path", type=str, default="logs/Rain100H/fusion_can_multiscale", help='path to save models and log files')
...
def main():
    ...
    # Build model
    model = fusion_can_multiscale(recurrent_iter=opt.recurrent_iter, use_GPU=opt.use_gpu)
    print_network(model)
    ...
    for epoch in range(initial_epoch, opt.epochs):
        ...
        for i, (input_train, target_train) in enumerate(loader_train, 0):
            ...
            print("[epoch %d][%d/%d] ssim_loss: %.4f, pixel_metric: %.4f, PSNR: %.4f" %
                  (epoch+1, i+1, len(loader_train), loss1.item(), pixel_metric.item(), psnr_train))
...
if __name__ == "__main__":
    if opt.preprocess:
        if opt.data_path.find('RainTrainH') != -1:
            ...
            prepare_data_Rain12600(data_path=opt.data_path, patch_size=100, stride=80)
        else:
