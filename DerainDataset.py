...
##    print("Total number of patches:", TotalPatNum)
...
def prepare_data_TrainHaug(data_path, patch_size, stride):
    # train
    print('process training data')
    input_path = os.path.join(data_path)
    target_path = os.path.join(data_path)
...
def prepare_data_CamvidL(data_path, patch_size, stride):
    # train
    print('process training data')
    input_path = os.path.join(data_path)
    target_path = os.path.join(data_path)
...
def prepare_data_DIDMDN(data_path, patch_size, stride):
    # train
    print('process training data')
    input_path = os.path.join(data_path)
    target_path = os.path.join(data_path)
...
##                target = torch.from_numpy(np.flip(target, axis=1).copy())
##                input = torch.from_numpy(np.flip(target, axis=1).copy())
##                
